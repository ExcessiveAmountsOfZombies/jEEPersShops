...
public class RemoveItemMenu extends ExpandedMenu implements ShopBack {

    private final Shop shop;
    private final int slotBeingPurchasedFrom;

    // 0 1 2 3 -4- 5 6 7 8
    public RemoveItemMenu(MenuType<ChestMenu> menuType, int containerId, Inventory inventory, int rows, Shop shop, int slotPurchase) {
    ...
    public SlotBuilder createSlot() {
        return (container, slotID, xPos, yPos) -> {
            if (slotID >= 0 && slotID < 4) {
                return new RemoveSlot(container, slotID, xPos, yPos, this);
            } else if (slotID == 4) {
    ...
    public void attemptPurchaseFromSlot(ServerPlayer player) {
        shop.attemptPurchase(slotBeingPurchasedFrom, player, true);
    }
